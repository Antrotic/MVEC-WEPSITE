...
import OrderPlayFillIcon from 'remixicon-react/OrderPlayFillIcon';
      ...
      {items.map((data, key) => {
        if (!cookies?.access_token) {
          if (
            ...
            data.path !== '/' &&
            data.path !== '/settings' &&
            data.path !== '/order-template' &&
            data.path !== '/my-gift-cards' &&
            data.path !== '/money-requests'
          ) {
