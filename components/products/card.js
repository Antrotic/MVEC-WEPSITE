...
function ProductCard({data, isGiftCard}) {
  ...
  return (
    <div
      ...
      {!isGiftCard && data.quantity >= data.min_qty && (
              ...
              <div className="counter-btn">
                <div
                  ...
                  onClick={() =>
                    decrease({
                      ...data,
                      user_cart_uuid: cartData.user_cart_uuid,
                    })
                  }>
                  <SubtractFillIcon />
                </div>
                <div className="count">{`${cartData.qty}`}</div>
                <div
                  className="increment"
                  onClick={() =>
                    increase({
      ...
      <Link href={`/product/${data?.product?.slug}`} className="product-name">
      ...
      <div className="product-card-footer">
        <div className="price">
          {!isGiftCard && (
            <span className="unit">{`${data?.product?.unit?.translation?.title} / `}</span>
          )}
          <span>
            {getPrice(data.discount ? data.price - data.discount : data.price)}
          </span>
        </div>
      </div>
      <Link href={`/product/${data?.product?.slug}`} className="product-name">
