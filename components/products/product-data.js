...
function ProdctData({setVisible, product}) {
  ...
  const currentProduct = cart.cartItems.find(item => item?.id === product?.id);
  ...
  const handleBuyGiftCard = async () => {
    ...
    try {
      ...
      const calcRes = await CartApi.calculate(cartRes.data.id, {
        currency_id: cookies.currency_id,
      });
      setCalculateResult({
        ...calcRes.data,
    ...
    <div className="product-detail-data">
      ...
      {isGiftCard && (
        <div className="my-3">
          ...
          <Modal
              ...
              <div className="d-block mt-5">
                <ul className="p-0 theme-text-black">
                  <li className="d-flex align-items-center justify-content-between border-bottom ">
                    ...
                    <span className="h5 fw-bolder my-1">
                      {getPrice(calculateResult.product_total)}
                    </span>
                  </li>
                  <li className="d-flex align-items-baseline justify-content-between border-bottom">
                    <span>Shop tax</span>
                    <span className="h5 fw-bolder my-1">
                      {getPrice(calculateResult.order_tax)}
                    </span>
                  </li>
                  <li className="d-flex align-items-baseline justify-content-between border-bottom">
                    <span>VAT tax</span>
                    <span className="h5 fw-bolder my-1">
                      {getPrice(calculateResult.product_tax)}
                    </span>
                  </li>
                  <li className="d-flex align-items-baseline justify-content-between border-bottom">
                    <span>Total amount</span>
                    <span className="h5 fw-bolder my-1">
                      {getPrice(calculateResult.order_total)}
                    </span>
                  </li>
                </ul>
              </div>
            ...
            <ModalFooter>
              <Button
                disabled={loadingState.continue}
                color="primary"
                onClick={() => {
                  createGiftOrder(
                    calculateResult.cart_id,
                    calculateResult.user_id,
                  );
                }}>
                Continue{' '}
      ...
      <div className="btn-box">
        {currentProduct ? (
          ...
          <button
            disabled={
              (isGiftCard && loadingState.sendGift) ||
              (product?.quantity >= product?.min_qty ? false : true)
            }
