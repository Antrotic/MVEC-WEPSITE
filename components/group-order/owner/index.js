...
import {clearCart, getTotals, setCartData} from '../../../redux/slices/cart';
...
function Owner() {
  ...
  const [buttonText, setButtonText] = useState(tl('Copy link'));
  const [loader, setLoader] = useState(false);
  const toggle = () => dispatch(setOpengomodal(!opengomodal));
  const handleCopyLink = () => {
    setButtonText(tl('Link copied'));
  ...
  const clearAll = () => {
    batch(() => {
      ...
      dispatch(getTotals(cratData?.shop_id));
    });
    setButtonText(tl('Copy link'));
  ...
  return (
    <>
      <ModalHeader toggle={toggle}>{tl('Together order')}</ModalHeader>
      <ModalBody>
        <Card>
          <CardBody>
